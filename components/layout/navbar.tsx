...
export function Navbar({ title }: NavbarProps) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 h-16 bg-background/80 backdrop-blur-md z-50 shadow-sm pr-[var(--removed-body-scroll-bar-size)]">
            ...
            <NavigationMenuList>
              {siteConfig.mainNav.map((item: NavItem) => (
                <NavigationMenuItem key={item.href || `menu-item-${item.title}`}>
                  {item.items ? (
                    <NavigationMenuTrigger className="h-9 px-4 text-sm font-medium text-foreground transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground data-[active]:bg-accent data-[active]:text-accent-foreground data-[state=open]:bg-accent">
                    ...
                    <Link href={item.href || '#'} legacyBehavior passHref>
                      <NavigationMenuLink className={cn(
                        navigationMenuTriggerStyle(),
                        "h-9 px-4 text-sm font-medium text-foreground transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground"
                      )}>
                        <span className="flex items-center gap-2">
                          {item?.icon && <item.icon className="w-4 h-4" />}
                          {item.title}
                            ...
                            <NavigationMenuLink asChild>
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  {subItem.icon && <subItem.icon className="h-4 w-4 text-foreground" />}
      ...
      {/* Mobile Menu */}
      <div className={cn(
        "md:hidden fixed inset-0 top-16 bg-background z-50 transition-transform duration-300 ease-in-out",
